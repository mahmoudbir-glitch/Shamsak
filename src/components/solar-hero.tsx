...
function FlowLine({
  ...
  return (
    ...
    >
      ...
      {active && (
        <circle
          cx={direction === "left" || direction === "right" ? "50" : "50"}
          cy={direction === "up" || direction === "down" ? "50" : "50"}
          r="3"
          fill={color}
...
const nodeTones = {
  solar:
    "border-amber-400/40 bg-slate-900/95 text-amber-300 shadow-[0_0_28px_rgba(245,158,11,.14)]",
  home:
    "border-cyan-400/40 bg-slate-900/95 text-cyan-300 shadow-[0_0_28px_rgba(34,211,238,.14)]",
  battery:
    "border-emerald-400/40 bg-slate-900/95 text-emerald-300 shadow-[0_0_28px_rgba(16,185,129,.14)]",
  grid:
    "border-violet-400/40 bg-slate-900/95 text-violet-300 shadow-[0_0_28px_rgba(139,92,246,.14)]",
} as const;
...
function Node({
  ...
  return (
    <div
      className={`relative z-20 flex h-[132px] w-[126px] flex-col items-center justify-center rounded-2xl border p-2.5 text-center backdrop-blur transition-transform duration-300 hover:-translate-y-0.5 sm:h-[154px] sm:w-[158px] sm:p-3${nodeTones[tone]}`}
    >
      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 sm:h-11 sm:w-11">
        {icon}
      </div>
...
function BatteryNode({
  ...
  return (
    <div className="relative z-20 flex h-[132px] w-[126px] flex-col items-center justify-center rounded-2xl border border-emerald-400/40 bg-slate-900/95 p-2.5 text-center text-emerald-300 shadow-[0_0_28px_rgba(16,185,129,.14)] backdrop-blur transition-transform duration-300 hover:-translate-y-0.5 sm:h-[154px] sm:w-[158px] sm:p-3">
      <div className="relative h-[66px] w-[66px] shrink-0" aria-label={`نسبة البطارية ${safe}%`}>
        ...
        <span className="absolute inset-0 flex items-center justify-center text-base font-black text-white">
          {safe}%
        </span>
      </div>
...
export function SolarHero({ data }: SolarHeroProps) {
  ...
  return (
    <section
      ...
      className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-950 shadow-[0_20px_70px_rgba(2,6,23,.22)]"
    >
      ...
      <div className="relative overflow-hidden px-3 py-6 sm:px-6 sm:py-8">
        ...
        <div className="relative mx-auto max-w-[900px]">
          ...
          <div
            className="mx-auto grid w-full max-w-[820px] grid-cols-[minmax(126px,1fr)_54px_minmax(80px,1fr)_54px_minmax(126px,1fr)] grid-rows-[auto_54px_auto_54px_auto] items-center justify-items-center gap-2 sm:grid-cols-[minmax(158px,1fr)_78px_minmax(110px,1fr)_78px_minmax(158px,1fr)] sm:grid-rows-[auto_78px_auto_78px_auto] sm:gap-3"
          >
            ...
            <div className="col-start-2 row-start-3 flex h-full w-full items-center justify-center">
              <FlowLine direction="right" active={gridImport || gridExport} color="#8b5cf6" />
            </div>
            ...
            <div className="col-start-3 row-start-3 flex h-[88px] w-[88px] items-center justify-center rounded-full border border-blue-300/20 bg-slate-900 shadow-[0_0_35px_rgba(59,130,246,.25)] sm:h-[104px] sm:w-[104px]">
              <div className="flex h-[66px] w-[66px] items-center justify-center rounded-full border border-blue-400/20 bg-blue-500/10 sm:h-[78px] sm:w-[78px]">
                <Zap className="text-blue-300" size={30} aria-hidden="true" />
              </div>
            </div>

            <div className="col-start-4 row-start-3 flex h-full w-full items-center justify-center">
              <FlowLine direction="right" active={data.homePowerW > 50} color="#22d3ee" />
            </div>
            ...
            <div className="col-start-3 row-start-4 flex h-full w-full items-center justify-center">
              <FlowLine direction="down" active={charging || discharging} color="#34d399" />
            </div>
