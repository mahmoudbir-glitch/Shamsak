...
export default function MoneyPage(){
 ...
 const hasDaily=period==="day" && (demoSnapshot.todayProductionKWh!=null || demoSnapshot.todayHomeUsageKWh!=null || demoSnapshot.todayGridSavings!=null);
 const solar=period==="day" ? demoSnapshot.todayProductionKWh : undefined; const usage=period==="day" ? demoSnapshot.todayHomeUsageKWh : undefined; const savings=period==="day" ? demoSnapshot.todayGridSavings : undefined;
 ...
 return <div className="space-y-5">
  <header><p className="text-xs font-bold text-violet-600">المال</p><h1 className="mt-1 text-2xl font-extrabold">المال والتوفير</h1><p className="mt-1 text-sm text-slate-500">التكاليف تعتمد على القراءات المتاحة والتعرفة التي تحددها من الإعدادات. الفترات الأسبوعية والشهرية تحتاج سجلًا تاريخيًا حقيقيًا.</p></header>
  ...
  <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><PiggyBank className="text-emerald-600" size={21}/><h2 className="font-black">المال والتوفير</h2></div>
   ...
   <p className="mt-3 text-xs text-slate-500">ملاحظة: لا يتم تحويل قراءة يوم واحد إلى أسبوع أو شهر. عند توفر سجل تاريخي حقيقي يمكن حساب الفترات الأطول بدقة.</p>
  </section>
 </div>;
}
